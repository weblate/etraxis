 ...
 * Command handler.
 */
final class WatchIssueCommandHandler implements CommandHandlerInterface
{
    /**
     ...
     */
    public function __invoke(WatchIssueCommand $command): void
    {
        ...
        if (!$this->security->isGranted(IssueVoter::VIEW_ISSUE, $issue)) {
            throw new AccessDeniedHttpException('You are not allowed to watch this issue.');
        }

        /** @var null|Watcher $watcher */
        $watcher = $this->watcherRepository->findOneBy([
            'user'  => $user,
