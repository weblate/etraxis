 ...
 * @internal
 *
 * @coversDefaultClass \App\Entity\FieldStrategy\StringStrategy
 */
final class StringStrategyTest extends TransactionalTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        [/* skipping */ , $this->field] = $repository->findBy(['name' => 'Commit ID']);
    ...
    public function testDefaultValue(): void
    {
        ...
        $huge = str_pad('', StringStrategy::MAX_LENGTH + 1);
    ...
    {
        $value = random_int(StringStrategy::MIN_LENGTH, StringStrategy::MAX_LENGTH);
        $min   = StringStrategy::MIN_LENGTH - 1;
    ...
    {
        $this->strategy->setParameter(Field::LENGTH, 12);
        $this->strategy->setParameter(Field::PCRE_CHECK, '(\d{3})-(\d{3})-(\d{4,5})');

        $errors = $this->validator->validate('123-456-7890', $this->strategy->getValueValidationConstraints($this->translator));
        self::assertCount(0, $errors);

        $errors = $this->validator->validate('123-456-78901', $this->strategy->getValueValidationConstraints($this->translator));
